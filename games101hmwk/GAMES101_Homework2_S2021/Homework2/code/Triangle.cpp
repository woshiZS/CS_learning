...
std::array<Vector4f, 3> Triangle::toVector4(const Vector3f& zView) const
{
    std::array<Eigen::Vector4f, 3> res;
    int i = 0;
    std::transform(std::begin(v), std::end(v), res.begin(), [zView, i](auto& vec) { return Eigen::Vector4f(vec.x(), vec.y(), vec.z(), zView(i, 0)); });
    return res;
}